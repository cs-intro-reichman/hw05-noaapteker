...
//		 test3(fileName, 3);
	...
	private static void test2(String fileName) {
		int[][] board = read(fileName);
		if (fileName.equals("square.dat")) {
			System.out.println("The next value of (2 , 2) should be 1 and is: " + cellValue(board, 2,  2));
		}
		if (fileName.equals("line.dat")) {
			System.out.println("The next value of (3 , 2) should be 0 and is: " + cellValue(board, 3,  2));
		}
	}
	...
	public static int[][] read(String fileName) {
		...
		for (int i = 1; i <= rows; i++) {
			...
			if (row == "") {
				...
				for (int j=row.length()+1; j<=cols; j++) {
					board[i][j] = 0;
				}
			}
		}
	...
	public static int[][] evolve(int[][] board) {
		...
		for (int i = 1; i <= nextBoard.length-2; i++) {
			for (int j = 1; j <=nextBoard[0].length-2; j++) {
	...
	public static int cellValue(int[][] board, int i, int j) {
		int neigh = count(board, i, j);
		if (board[i][j] == 1 && neigh < 2) {
			return 0;
		}
		if (board[i][j] == 1 && neigh >=2 && neigh <=3) {
			return 1;
		}
		if (board[i][j] == 1 && neigh > 3) {
			return 0;
		}
    ...
    public static void print(int[][] arr) {
		for (int i=1; i <= arr.length-2; i++) {
			for (int j=1; j<= arr[1].length-2; j++) {
